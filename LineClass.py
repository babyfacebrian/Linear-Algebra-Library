...
class Line(object):
    ...
    def is_parallel(self, line2):
        return 'Parallel: {}'.format(self.normal_vector.is_parallel(line2.normal_vector))
    ...
    def __eq__(self, line2):
        ...
        if not self.normal_vector.is_parallel(line2.normal_vector):
            return False
        basepoint_diff = self.basepoint.__sub__(line2.basepoint)
    ...
    def intersection(self, line2):
        ...
        try:
            x_int = (D*k1 - B*k2) / (A*D - B*C)
            y_int =(-C*k1 + A*k2) / (A*D - B*C)
            return Vector([x_int, y_int])
        except ZeroDivisionError:
            if self == line2:
                raise Exception("Lines are equal")
            else:
